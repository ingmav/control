<?php
	...
	if($_POST["accion"] == "index")
	{
		...
		$condicionales = " AND DOCTOS_VE.FECHA > '2020-01-01' AND (DOCTOS_VE.TIPO_DOCTO='F' OR DOCTOS_VE.TIPO_DOCTO='R') AND DOCTOS_VE.ESTATUS!='C' AND DOCTOS_VE.DOCTO_VE_ID NOT IN (SELECT DOCTO_VE_ID FROM TABLEROPRODUCCION WHERE CERRAR_SELECCION=1) AND DOCTOS_VE.DOCTO_VE_ID  NOT IN (SELECT DOCTOS_VE_LIGAS.DOCTO_VE_DEST_ID FROM DOCTOS_VE_LIGAS, DOCTOS_VE WHERE DOCTOS_VE_LIGAS.DOCTO_VE_FTE_ID=DOCTOS_VE.DOCTO_VE_ID AND DOCTOS_VE.TIPO_DOCTO='R')
				 ";
		...
		{
			$condicionales.= " AND CLIENTES.NOMBRE like '%".strtoupper(utf8_decode($_POST['client']))."%'";
		}
	...
	if($_POST["accion"] == "index_mostrador")
	{
	   ...
	   $query = "select
		...
		while ($row = ibase_fetch_object ($result, IBASE_TEXT)){
			...
			$json_mostrador[$indice]['NOMBRE_CLIENTE'] 	= utf8_decode($row->NOMBRE_CLIENTE);
			$json_mostrador[$indice]['DESCRIPCION'] 	= utf8_decode($row->DESCRIPCION);
	...
	if($_POST["accion"] == "eliminar_folios")
	{
		$contador = count($_POST['folio']);
		$count = 0;
		$join = array();
		//print_r($_POST['folio'][0]);
		if($_SESSION['IDUSUARIO']== 21)
		{
			...
			{	
				...
				$valores = array($_POST['folio'][$count],0,1, $diseno, $impresion, $entrega, $maquilas, $instalacion, 1, "''", "'".date("Y-m-d H:i:s")."'", $preparacion, "'".$fecha_entrega." ".$hora_entrega."'", 1, 1);
