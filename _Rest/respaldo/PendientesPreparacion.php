<?php
	...
	if($_POST["accion"] == "index")
	{
		...
		{
			$consulta1 .= " AND TABLEROPRODUCCION.PREPARACION_GF=2 AND TABLEROPRODUCCION.F_PREPARACION_GF LIKE '".$_POST['fecha']."%' ";
		}else
		{
			$consulta1 .= " AND TABLEROPRODUCCION.PREPARACION_GF!=2 ";
		}
		...
		(SELECT ALIAS FROM OPERADOR WHERE OPERADOR.ID = TABLEROPRODUCCION.OPERADOR_PREPARACION_GF) AS NOMBRE_OPERADOR,
		TABLEROPRODUCCION.NOTA,
		TABLEROPRODUCCION.ACTIVACION,
		(select count(*) from TABLEROOBSERVACION where TABLEROOBSERVACION.IDTABLEROPRODUCCION=TABLEROPRODUCCION.ID AND TABLEROOBSERVACION.IDDEPARTAMENTO=9) as CONTADOR_MESSAGE
        ...
        AND TABLEROPRODUCCION.GF_PREPARACION=1 AND PREPARACION_GF!=2
        AND (TABLEROPRODUCCION.GF_DISENO=0 OR TABLEROPRODUCCION.DISENO_GF=2)
        AND (TABLEROPRODUCCION.GF_IMPRESION=0 OR TABLEROPRODUCCION.IMPRESION_GF=2) ".$consulta1;
		...
		if($_POST['realizados'] && $_POST['realizados']==2)
		{
			$consulta2 .= " AND PRODUCCIONPV.PREPARACION_GF=2 AND PRODUCCIONPV.F_PREPARACION_GF LIKE '".$_POST['fecha']."%' ";
		}else
		{
			$consulta2 .= " AND PRODUCCIONPV.PREPARACION_GF!=2 ";
		}
		...
		(SELECT ALIAS FROM OPERADOR WHERE OPERADOR.ID = PRODUCCIONPV.OPERADOR_PREPARACION_GF) AS NOMBRE_OPERADOR,
		PRODUCCIONPV.IDESTATUSIMPRESION,
		PRODUCCIONPV.DESCRIPCION,
		PRODUCCIONPV.ACTIVACION,
		(select count(*) from PVOBSERVACION where DOCTOS_PV.DOCTO_PV_ID = PVOBSERVACION.DOCTO_PV_ID AND PVOBSERVACION.IDDEPARTAMENTO=9) as CONTADOR_MESSAGE
		from DOCTOS_PV,
       	PRODUCCIONPV
        WHERE  DOCTOS_PV.TIPO_DOCTO='V' AND DOCTOS_PV.ESTATUS!='C' 
        AND PRODUCCIONPV.DOCTO_PV_ID = DOCTOS_PV.DOCTO_PV_ID 
        AND PRODUCCIONPV.PREPARACION_GF!=2 AND PRODUCCIONPV.GF_PREPARACION=1
