<?php
	...
	if($_POST["accion"] == "abrirdocumento")
	{
		...
		{
			$conection2 = new conexion_nexos($_POST['empresa']);
			$campos = array("CERRADOPRODUCCION.ID");
			$join = array("TABLEROPRODUCCION","=", "CERRADOPRODUCCION.DOCTO_VE_ID", "TABLEROPRODUCCION.DOCTO_VE_ID");
			$condicionales = " AND TABLEROPRODUCCION.ID=".$_POST['id'];

			$json = $conection2->select_table($campos, "CERRADOPRODUCCION", $join , $condicionales, array(), 0);
			//print_r($json[0]['CERRADOPRODUCCION.ID']);
			$json2 = $conection2->delete_of_table("CERRADOPRODUCCION", "CERRADOPRODUCCION.ID=".$json[0]['CERRADOPRODUCCION.ID'],array());
