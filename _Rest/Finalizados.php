<?php
    ...
    if($_POST["accion"] == "index")
	{
        ...
        $query =    "select 
                    ...
                    IIF(F_DISENO_GF IS null, 'NO FINALIZADO', F_DISENO_GF) AS FECHA_DISENO,
                    GF_PREPARACION,
                    IIF(GF_PREPARACION=1, IIF(PREPARACION_GF=2, 1,0),0) AS ESTATUS_PREPARACION,
                    IIF(F_PREPARACION_GF IS null, 'NO FINALIZADO', F_PREPARACION_GF) AS FECHA_PREPARACION,
                    GF_IMPRESION,
                    IIF(GF_IMPRESION=1, IIF(IMPRESION_GF=2, 1,0),0) AS ESTATUS_IMPRESION,
                    IIF(F_IMPRESION_GF IS null, 'NO FINALIZADO', F_IMPRESION_GF) AS FECHA_IMPRESION,
                    GF_INSTALACION,
                    IIF(GF_INSTALACION=1, IIF(INSTALACION_GF=2, 1,0),0) AS ESTATUS_INSTALACION,
                    IIF(F_INSTALACION_GF IS null, 'NO FINALIZADO', F_INSTALACION_GF) AS FECHA_INSTALACION,
                    GF_ENTREGA,
                    IIF(GF_ENTREGA=1, IIF(ENTREGA_GF=2, 1,0),0) AS ESTATUS_ENTREGA,
                    IIF(F_ENTREGA_GF IS null, 'NO FINALIZADO', F_ENTREGA_GF) AS FECHA_ENTREGA,
                    GF_MAQUILAS,
                    IIF(GF_MAQUILAS=1, IIF(MAQUILAS_GF=2, 1,0),0) AS ESTATUS_MAQUILAS,
                    IIF(F_MAQUILAS_GF IS null, 'NO FINALIZADO', F_MAQUILAS_GF) AS FECHA_MAQUILAS,
                    ...
                    TABLEROPRODUCCION.FECHA_ENTREGA AS FECHA_ENTREGA_TRABAJO,
        ...
        while ($row = ibase_fetch_object ($result, IBASE_TEXT)){
            ...
            $json[$indice]['FOLIO']                 = substr($row->FOLIO,1);
            ...
            $json[$indice]['FECHA_MAQUILAS']        = $row->FECHA_MAQUILAS;
            $json[$indice]['NUMERO_FACTURA']        = ($row->NUMERO_FACTURA != null) ? "NPF-".(int)substr($row->NUMERO_FACTURA,1) : '';
            $json[$indice]['FECHA_ENTREGA_TRABAJO'] = $row->FECHA_ENTREGA_TRABAJO;
            $json[$indice]['DOCTO_VE_ID']           = $row->DOCTO_VE_ID;
        } 
	...
	if($_POST['accion'] == "informacion")
	{
		...
		$campos = array("GF_DISENO", "GF_IMPRESION", "GF_MAQUILAS", "GF_INSTALACION", "GF_ENTREGA", "GF_PREPARACION", "DISENO_GF", "IMPRESION_GF", "MAQUILAS_GF", "INSTALACION_GF", "ENTREGA_GF", "PREPARACION_GF");
    ...
    if($_POST['accion'] == "informacion_extra")
    {
        ...
        $join = array("DOCTOS_VE_DET","=", "DOCTOS_VE_DET.DOCTO_VE_ID", "DOCTOS_VE.DOCTO_VE_ID",
                      "ARTICULOS","=", "DOCTOS_VE_DET.ARTICULO_ID", "ARTICULOS.ARTICULO_ID");

        $condicionales = " AND DOCTOS_VE.DOCTO_VE_ID=".$_POST['id']." AND DOCTOS_VE_DET.ROL!='C'";
