<?php
	...
	if($_POST["accion"] == "ventas")
	{
	    ...
	    $condicionales = " and tipo_docto in ('F', 'R') and ESTATUS!='C' AND fecha $fecha1 and docto_ve_id not in (select dvl.docto_ve_fte_id from  doctos_ve_ligas dvl)";
	    $arreglo1 = $conection->sum_regular("DOCTOS_VE.IMPORTE_NETO", "", "DOCTOS_VE", array(), $condicionales, $softdelete);

	    //$condicionales_1 = " and tipo_docto in ('F','R') and ESTATUS!='C' AND fecha = $fecha2 and docto_ve_id not in (select dvl.docto_ve_fte_id from  doctos_ve_ligas dvl)";
	    $condicionales_1 = " and tipo_docto in ('F', 'R') and ESTATUS!='C' AND fecha = $fecha2 and docto_ve_id not in (select dvl.docto_ve_fte_id from  doctos_ve_ligas dvl)";
    ...
    if($_POST['accion'] == "calendarioServicio")
    {
        ...
        /**/
        $tabla = "DOCTOS_VE";
        //$condicionales = " and tipo_docto in ('F', 'R') and docto_ve_id not in (select dvl.docto_ve_fte_id from  doctos_ve_ligas dvl) and ESTATUS!='C' AND fecha between '".date("Y-m-")."01' and '".date("Y-m-d")."' group by fecha order by fecha";
        $condicionales = " and tipo_docto in ('F', 'R') and docto_ve_id not in (select dvl.docto_ve_fte_id from  doctos_ve_ligas dvl) and ESTATUS!='C' AND fecha between '".date("Y-m-")."01' and '".date("Y-m-d")."' group by fecha order by fecha";
