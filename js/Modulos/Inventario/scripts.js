...
function InicializaInventario()
{
    ...
    RestFullRequest("_Rest/Inventario.php", variable, "verificaInicializacion", 1);
}	
...
function datagridInventario(Response)
{
	...
	$.each(Response, function(index, value)
	{
        ...
        if(value['LINEA_ID'] == 1351 || value['LINEA_ID'] == 2615)
...
{
    //console.log(Response);
    $("#tituloInventario").html("INVENTARIO NO. <span style='color:#E21800'><strong>"+Response[1]+"</strong></span> DEL <span style='color:#E21800'><strong>"+Response[0]['FECHA_INICIO']+"</strong></span> AL <span style='color:#E21800'><strong>"+Response[0]['FECHA_FIN']+"</strong></span>");
}
...
function guardarArticulo()
{
    ...
    RestFullRequest("_Rest/Inventario.php", variable, "recargaLista", 1);
}

function recargaLista()
{
    //console.log($("#linefilter").val());
...
function rellenarReajuste(Response)
{
	...
	$.each(Response, function(index, value)
	{
		...
		$("#reajustearticulos").append("<tr "+color+"><td>"+value['NOMBRE']+"</td><td>"+value['UNIDAD']+"</td><td>"+value['LINEA']+"</td><td><input type='text' name='cantidad_"+value['ID']+"' value='"+value['TOTAL']+"'></td><td><input type='checkbox' name='ARTICULOSWEB[]' value='"+value['ID']+"' checked='checked'></td></tr>");
		contador++;
	});
}
...
function agregarReajuste()
{
    ...
    RestFullRequest("_Rest/Inventario.php", variable, "cargasubarticulos", 1);
    //verificaInicializacion();
}

function cargasubarticulos(response)
{
    //console.log(response);
...
function agregarsubarticulos()
{
    ...
    RestFullRequest("_Rest/Inventario.php", variable, "finalizacorte", 1);
}
...
{
	//window.open("reportes/Inventario/ReporteInventario.php", "_blank");
    $("#Config_Reporte").modal("show");
    var variable = "accion=cargaCortes";
    RestFullRequest("_Rest/Inventario.php", variable, "cargaCorte");
}

function cargaCorte(Response)
{
    //console.log(Response);
    $("#seleccion_corte").html("");
    $.each(Response, function(index, value)
    {
        if(value['FECHA_FIN'] == "")
            value['FECHA_FIN'] = "ACTUAL";
        $("#seleccion_corte").append("<option value='"+value['ID']+"'>"+value['INVENTARIO_NO']+" - DE "+value['FECHA_INICIO']+" A "+value['FECHA_FIN']+"</option>")
    });
}
