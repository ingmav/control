		...
		$( "#numeroCotizacion" ).keypress(function(e) {
			if(e.keyCode == 13)
			{
				...
				var variable = "accion=index&buscar="+$("#numeroCotizacion").val()+"&page="+paginacion+"&filtroEstatus="+filtroEstatus;
		...
		function actualizaDatagrid()
		{
			...
			var variable = "accion=index&buscar="+$("#numeroCotizacion").val()+"&filtroEstatus="+filtroEstatus+"&page="+paginacion;
            RestFullRequest("_Rest/SeguimientoCotizacion.php", variable, "datagridCotizacion");
		}
		...
		function paginador()
		{
			...
			var variable = "accion=counter&buscar="+$("#numeroCotizacion").val()+"&filtroEstatus="+filtroEstatus;
		...
		function datagridCotizacion(response)
		{
			...
			$.each(response, function(index, value)
			{
				...
				campos += "<td>"+combo+"</td>";
                campos += "<td>"+value['NOMBREEMPRESA']+"-"+(parseInt(value['DOCTOS_VE.FOLIO']))+" - "+value['DOCTOS_VE.ESTATUS']+"</td>";
                campos += "<td>"+value['DOCTOS_VE.FECHA']+"</td>";
                campos += "<td>"+value['SEGUIMIENTOCOTIZACION.MODIFICADO_AL']+"</td>";
		...
		function reporte()
		{
			...
			window.open('ReporteCotizacion.php?tipo='+$("#estatus").val(),'_blank')
		}
		function cambiaBuscador(valor)
		{
			filtroEstatus = valor;
