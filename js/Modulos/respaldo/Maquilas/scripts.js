...
function datagridPendientes(response)
{
	...
	actualizaProcesos();
	//paginador();
	var datagrid = $("#PendientesMaquilas");
	...
	$.each(response, function(index, value)
	{
		...
		if($("#realizados").val() == 1)
		{
			...
			var orden_dia = "<button type='button' class='btn  btn-strech btn-warning' onClick='desactivaProceso("+id+", "+value['EMPRESA']+", 8)'><i class='fa fa-spin fa-cog'></i></button>";
			else
			var orden_dia = "<button type='button' class='btn btn-strech' onClick='activaProceso("+id+", "+value['EMPRESA']+", 8)'><i class='fa fa-circle'></i></button>";
		...
		campos += "<td style='text-align:justify'><b style='color:blue'>"+value['CLIENTES.NOMBRE']+"</b><b> ("+parseFloat(value['DOCTOS_VE_DET.UNIDADES']).toFixed(2)+" "+value['ARTICULOS.UNIDAD_VENTA']+")</b><br>"+texto_descripcion+"<br></td>";
		...
		//var cancelar = "<button type='button' class='btn btn-circle  btn-danger' title='CANCELAR' onClick='cancelar(this, "+value['EMPRESA']+")'><i class='fa fa-close'></i></button>";
...
function activaProceso(idProceso, emp, tipo)
{
    ...
    RestFullRequest("_Rest/PendientesMaquilas.php", variable, "actualizaDatagrid");
}
...
function buscarRealizados()
{
    ...
    RestFullRequest("_Rest/PendientesMaquilas.php", variable, "datagridPendientes");
...
function contadorMensajes(id, emp)
{
    ...
    RestFullRequest("_Rest/PendientesMaquilas.php", variable, "cargaCountMessaje");
}
...
function observaciones(obj, emp)
{
    ...
    RestFullRequest("_Rest/PendientesMaquilas.php", variable, "cargaObservaciones");
}
...
function operadores()
{
    ...
    RestFullRequest("_Rest/PendientesMaquilas.php", variable, "cargaOperadores");
}

function guardarObservacion()
{
	var variable = "accion=saveObservacion&empresa="+empresa+"&"+$("#formObservaciones").serialize();
    RestFullRequest("_Rest/PendientesMaquilas.php", variable, "actualizaDatagrid", 1);
